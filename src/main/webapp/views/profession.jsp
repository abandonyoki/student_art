			...
			<div class="panel panel-default">
				...
				<form id="searchForm" class="form-horizontal">
					<div class="form-group">
						<label class="col-md-1 control-label" for = "professionName">专业名称</label>
						<div class="col-md-2">
							<input class="form-control" type="text" name="professionName">
						</div>
						<label class="col-md-1 control-label" for = "serieId">学院名称</label>
						<div class="col-md-2">
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<form id="setForm" class="form-horizontal">
					...
					<div class="form-group">
						<label class="col-sm-4 control-label" for="profession_name">专业名称</label>
						<div class="col-sm-6">
							<input class="form-control" type="text" name = "professionName" placeholder="请输入专业名称">
						</div>
					</div>
					<!--修改-->
					<div class="form-group">
						<label class="col-sm-4 control-label" for="serieId">学院名称</label>
						<div class="col-sm-6">
...
var operation={
	...
	addForm:function(){
		...
		if($("#setForm").data("bootstrapValidator").isValid()){
			...
			$.post("profession/insertProfession", temp ,function(data) {
				var obj = eval('('+data+')');
				$("#dg").bootstrapTable("refresh");
	...
	editForm:function(){
		...
		if($("#setForm").data("bootstrapValidator").isValid()){
			var temp = getFormJson("setForm");
			$.post("profession/updateProfession", temp ,function(data) {
				var obj = eval('('+data+')');
				$("#dg").bootstrapTable("refresh");
				message.alert(obj.showInfo);
				$('#setForm').data('bootstrapValidator').resetForm(true);
	...
	deleteFrom:function(){
		...
		if(rows.length == 1  ){
			bootbox.confirm({
				size:"small",
				title:"提示",
				message:"这将会删除相关信息，确认删除这条数据吗？",
				callback:function(result){
					if(result){
						$.post("profession/deleteProfession", {id:rows[0].id} ,function(data) {
							var obj = eval('('+data+')');
							$("#dg").bootstrapTable("refresh");
